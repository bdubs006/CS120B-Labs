...
int main(void)
{
    ...
    while (1)
    {
	    ...
	    switch(fuellvl){ //your condition, like if(fuellvl = ???)
		    case 0: // if the fuel level equals to 0
		    sensorLEDS = 0x40; //10 0000
		    break;
		    case 1: //if equals to 1 (the ??? part, so if(Fuellvl = 1)
		    ...
		    break;
		    if ((GetBit(PINA, 4) && GetBit(PINA, 5) && ~GetBit(PINA, 6)) == 1) {
				sensorLEDS = (sensorLEDS & 0x7F) | 0x8A;
			}
			else {
				sensorLEDS = (sensorLEDS & 0x7F) | 0x00;
			}
	    }
	    PORTC = sensorLEDS; //how you output to the display, assign the output to your variable
